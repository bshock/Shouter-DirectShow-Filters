...
int sock_valid_socket(sock_t sock)
{
    ...
    ret = getsockopt(sock, SOL_SOCKET, SO_TYPE, (char*) &optval, &optlen);
...
int sock_set_nolinger(sock_t sock)
{
    ...
    return setsockopt(sock, SOL_SOCKET, SO_LINGER, (char *)&lin, 
...
int sock_set_nodelay(sock_t sock)
{
    ...
    return setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (char *)&nodelay,
            sizeof(int));
}
...
{
    int keepalive = 1;
    return setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, (char *)&keepalive, 
            sizeof(int));
}
...
size_t sock_writev (sock_t sock, const struct iovec *iov, size_t count)
{
...
*/
int sock_write_bytes(sock_t sock, const char *buff, size_t len)
{
    /* sanity check */
    if (!buff) {
        return SOCK_ERROR;
...
int sock_connected (sock_t sock, int timeout)
{
    ...
    switch (select(sock + 1, NULL, &wfds, NULL, timeval))
    {
        ...
        default:
            /* on windows getsockopt.val is defined as char* */
            if (getsockopt(sock, SOL_SOCKET, SO_ERROR, (char*) &val, &size) == 0)
            {
                if (val == 0)
                    return 1;
                sock_set_error (val);
