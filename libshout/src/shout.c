...
int shout_close(shout_t *self)
{
	...
	if (self->state == SHOUT_STATE_CONNECTED && self->close_shout_t)
	{
		self->close_shout_t(self);
	}
...
int shout_send(shout_t *self, const unsigned char *data, size_t len)
{
	...
	return self->send_shout_t(self, data, len);
}

size_t shout_send_raw(shout_t *self, const unsigned char *data, size_t len)
{
	size_t ret;
...
size_t shout_queuelen(shout_t *self)
{
	if (!self)
		return SHOUTERR_INSANE;

	return (size_t)self->wqueue.len;
}
...
int shout_set_public(shout_t *self, unsigned int public_stream)
{
	if (!self || (public_stream != 0 && public_stream != 1))
		return SHOUTERR_INSANE;
...
static int queue_data(shout_queue_t *queue, const unsigned char *data, size_t len)
{
	...
	if (!queue->len) {
		queue->head = (shout_buf_t*)calloc(1, sizeof (shout_buf_t));
		if (! queue->head)
			return SHOUTERR_MALLOC;
	}
	...
	while (len > 0) {
		if (buf->len == SHOUT_BUFSIZE) {
			buf->next = (shout_buf_t*)calloc(1, sizeof (shout_buf_t));
			if (! buf->next)
				return SHOUTERR_MALLOC;
			buf->next->prev = buf;
			buf = buf->next;
...
static int parse_http_response(shout_t *self)
{
	...
	if (httpp_parse_response(parser, header, hlen, self->mount)) {
		retcode = (char*)httpp_getvar(parser, HTTPP_VAR_ERROR_CODE);
		code = atoi(retcode);
